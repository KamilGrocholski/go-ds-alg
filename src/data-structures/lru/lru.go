...
func CreateLru[K comparable, V any](capacity int) *Lru[K, V] {
	return &Lru[K, V]{capacity: capacity, lookup: make(map[K]*BiNode[V], capacity), head: nil, tail: nil, size: 0, reverseLookup: make(map[*BiNode[V]]K)}
}
...
func (lru *Lru[K, V]) prepend(node *BiNode[V]) {
	if lru.tail == nil {
		lru.head = node
		lru.tail = node
	} else {
...
func (lru *Lru[K, V]) detach(node *BiNode[V]) {
	if node.next != nil {
